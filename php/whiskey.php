<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Whiskeys</title>
    <link rel="stylesheet" href="Styles/style5.css">
</head>
<body>
    <!-- Navigation Section -->
    <nav class="nav">
        <div class="container">
            <a href="products.html" class="btn">Back to Alcohol...</a>
        </div>
    </nav>

    <!-- Main Content -->
    <main class="content">
        <!-- Jazz Juke Box Section -->
        <section class="jukebox">
            <h2>Jazz Juke Box</h2>
            <audio controls>
                <source src="Music/jazz3.mp3" type="audio/mpeg">
            </audio>
        </section>

        <!-- Whiskeys Showcase Section -->
        <section class="whiskeys">
            <h2>Whiskeys</h2>
            <div class="whiskey-images">
                <div class="img-container">
                    <img src="Images/jackd.png" alt="Blue Label Whiskey">
                </div>
                <div class="img-container">
                    <img src="Images/bluelabel.png" alt="Jack Daniels Whiskey">
                </div>
                <div class="img-container">
                    <img src="Images/jameson.png" alt="Jameson Whiskey">
                </div>
            </div>
        </section>
    </main>

    <!-- Footer Section -->
    <footer class="footer">
        <div class="container">
            <p>&copy; 2024 The Liquor Lounge. All Rights Reserved.</p>
        </div>
    </footer>
</body>
</html>

<?php

?>